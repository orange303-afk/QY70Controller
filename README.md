...
- Mono/Poly, key assignment and Part Mode;
- Note Shift, Detune, velocity response, note limits and Dry Level;
- Vibrato and EG Decay;
- Mod Wheel, Pitch Bend and Channel Aftertouch routing;
...
The editor is divided into Voice, Part, Controllers, Pitch EG, Reverb, Chorus,
and Variation/Delay tabs. Effect parameter names follow the selected Yamaha
algorithm: for example Hall exposes Reverb Time, Diffusion and Initial Delay,
while Delay L/C/R exposes the three channel delays and feedback controls. A
raw label is retained only where the current algorithm-specific table has not
yet been mapped to a display name.
